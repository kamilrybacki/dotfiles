...
description: >
  ...
  MCP wrapper, or automation endpoint.
---
...
> **Provenance (homelab):** Sections 1–10 are vendored verbatim from
> [`kunchenguid/axi`](https://github.com/kunchenguid/axi) `.agents/skills/axi/SKILL.md`
> at tag **`axi-sdk-js-v0.1.8`** (MIT License, © kunchenguid). Section 11 ("Reject unknown
> flags") is a homelab addendum. Do **not** force TOON into MCP wire contracts — MCP keeps
> structured JSON; TOON applies only at CLI/text output boundaries.
...
## 11. Reject unknown flags (homelab addendum)

Unknown or misspelled flags must **fail loudly**, never be silently ignored. Silent
acceptance lets an agent believe an option took effect when it did not — the worst kind of
failure because it is invisible.

```
$ tasks list --stat closed
error: unknown flag --stat (did you mean --state?)
help: tasks list [--state open|closed] [--fields <a,b>]    # exit 2
```

- Parse strictly: an unrecognized flag is a **usage error → exit code 2**, in the same
  structured stdout format as other errors (see §6).
- Suggest the nearest valid flag when one is close (edit-distance match), so the agent can
  self-correct in one step.
- Do **not** pass unrecognized flags through to a wrapped tool — that leaks the dependency's
  surface and produces unpredictable behaviour. Validate against your own flag set first.
- Applies to values too: reject an out-of-range enum value with the allowed set, don't coerce.

---

## When this skill triggers

Load and apply AXI when you are **building, modifying, or reviewing** any agent-facing
surface: a CLI a coding agent will invoke via shell, an MCP server's text output, a wrapper
around another tool (`gh`, `kubectl`, `docker`), or an automation endpoint whose output an
agent parses. It does **not** apply to human-facing UIs or to MCP structured (JSON) wire
payloads — keep those as JSON.

## Required-principle checklist (homelab conformance)

An AXI-conformant tool satisfies all 13: (1) token-efficient TOON output, (2) minimal default
schemas, (3) truncation with total-size + `--full` escape, (4) pre-computed counts/aggregates,
(5) definitive empty states, (6) structured errors + meaningful exit codes, (7) idempotent
mutations, (8) no interactive prompts, (9) content-first default output, (10) contextual
next-step hints, (11) concise per-subcommand `--help`, (12) stdout/stderr separation, and
(13) rejection of unknown flags.
