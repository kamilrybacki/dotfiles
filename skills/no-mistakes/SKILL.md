...
> **Provenance:** vendored from `kunchenguid/no-mistakes` **v1.37.0** (MIT). Pinned.

**Telemetry:** always run with `NO_MISTAKES_TELEMETRY=off` (set globally by the ansible role).

**When to use it (homelab):** for any **coding project** change you intend to ship as a PR —
after you've made the change on a **feature branch**, run the gate before opening the PR. It is
**not** a general chat tool. Normal project tests remain the first validation layer; no-mistakes
wraps review→test→docs→lint→push→PR on top.

**No autonomous shipping:** `auto_fix.*` is pinned to `0` in the project template (no yolo — the
gate blocks at findings and waits for `no-mistakes axi respond`). no-mistakes only **opens PRs,
never merges** — human review/merge stays mandatory, and branch protection is never bypassed.

**Sensitive-path exclusions** (in `.no-mistakes.yaml` `ignore_patterns`): Helm charts, ArgoCD
apps, RBAC/NetworkPolicy, auth, DB migrations, SOPS/secret files, and prod infra are excluded
from the pipeline so the gate agent never autonomously edits or ships them — those changes go
through human review directly. See the project template at
`dev-tools/claude-code-setup/files/no-mistakes/.no-mistakes.yaml.template`.

**Report after a run:** branch · tests executed · no-mistakes outcome · PR URL (if opened) ·
residual risk · that the task is **review-required** (not "done" — human merge is a separate gate).
